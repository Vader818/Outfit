...
> 状态：已于 2026-07-15 按本计划完成；11/11 实施任务与 6/6 验收标准均有自动化和真实浏览器证据。

## 共同执行约束

- 依赖关系：硬依赖 M2、M3、M4；M5 仅为建议先行验证，不是硬依赖。
...
- [x] 写 trip/date/activity/约束校验和迁移测试；结束日期早于开始日期、超过 7 天和负上限必须被拒绝。
- [x] 实现 Trip CRUD、Day/Activity 编辑和 7 天天气快照。
- [x] 把推荐引擎暴露为可复用 generateCandidates(context, constraints, budget)，不复制另一套搭配规则。
- [x] 写优化器纯函数测试：天气/场合适配、maxShoes、maxGarments、三种 repeatPolicy、每次洗衣前核心单品可穿次数、洗衣日重置、同日一套/多套和不可行结果；断言规划前后全局 garment availability 完全不变。
- [x] 实现 beam search 与 explainOptimization()；输出每件衣物覆盖日期和被选择原因。
- [x] 在“计划与洞察”二级区域增加旅行入口，不增加主导航项。
- [x] 生成逐日搭配后允许用户锁定/替换某日单品，再局部重算后续日。
- [x] PackingChecklist 支持“已打包/穿在身上/不带”，状态保存在本地数据库。
- [x] 旅行完成后可把实际穿着批量写入 wear events；必须逐日确认，不自动假设计划已执行。
- [x] 扩展 OutfitExportV2，加入 trips、activities、selections 与 packing states；图片仍只由 M1 的显式 ZIP 备份携带。
- [x] 更新文档并完成全量验证。

### 验收标准

- [x] 1–7 天旅行可生成逐日搭配和去重后的衣物清单。
- [x] 算法把最大鞋数、最大衣物数、不可用状态、重复规则和独立活动搭配当作硬约束；无解时返回冲突而不是违规方案。
- [x] 同一天多个活动能明确共用或分别选择搭配，“健身 + 正式晚宴”等冲突场景测试通过。
- [x] 洗衣日只改变方案搜索中的模拟计数；取消或保存旅行计划都不会把现实衣物擅自设为干净/待洗。
- [x] 用户能看到每件衣物服务哪些日期/活动，并可手工替换。
- [x] 旅行计划和衣物图片不发送给第三方；只有用户明确启用天气时发送坐标。

### 完成证据

- M6 跨层专项：11 个文件、281/281；2026-07-15 独立复验后的最终全量 Vitest：44 个文件、572/572；Pytest：38/38；typecheck 与 lint 通过。
- migration 7、Trip 认证 API、显式天气依赖注入、优化器硬约束、装箱四态、实际穿着幂等事务、OutfitExportV2 `trip-capsule` 数组均有专项回归。
- 首轮真实浏览器在隔离数据库创建 3 天“健身 + 正式晚宴”旅行，生成 4 套逐日搭配和去重清单，完成锁定、同类替换、覆盖解释、必需品、装箱状态与逐套确认；极限 2 件方案明确返回 maxGarments 冲突和最小放宽建议。独立复验又覆盖跨旅行编辑目标、完成态只读、冻结空方案和已打包必需品。
- 390×844 与 1440×900 无页面级横向溢出，交互 label 不小于 44px，控制台 0 error/0 warning；浏览器请求仅为本机同源地址，未显式刷新天气时没有第三方请求。
- npm 生产/全量审计均为 0 漏洞，锁定 Python 依赖审计为 0，`pip check` 无破损；非破坏性生产构建位于 `output/build-m6-final-20260715-1017`。
- 独立复验修复了实际穿着绑定、原子日期/活动编辑、派生状态失效、受保护状态、局部重算前缀与确认、放宽建议真实性、跨旅行编辑/迟到 GET 竞态、完成态只读及装箱覆盖解释等缺口；新增回归均纳入上述 572 项测试。
- 最终非破坏性生产构建位于 `output/build-m6-reaccept-final-20260715`。全程未删除文件、未修改真实 `data/outfit.sqlite`；完整 QA 与独立复验数据/日志分别保留在 `output/qa-m6-final-20260715-100357` 和 `output/playwright/m6-reaccept-20260715-1108`。
